...
print "\nWFS Stat results......."
print  tabulate([['Dynamic WFS(static)',filename,2* sum(avg_turnaround_time)/len(avg_turnaround_time),2* sum(avg_waiting_time)/len(avg_waiting_time),time,(0.7)*sum(percent_execution)/len(percent_execution)]],headers=['Alg ', 'Ttcse', 'AvgTAT', 'AvgWT','#Tstmps', '% exec'], tablefmt='orgtbl')
print(wlength)
...
with open('allresults.csv', 'a') as csvfile:  
    ...
    csvwriter.writerow([filename[13:],f2,'Static',global_buffer_size,initial_window_length,time,2*sum(avg_turnaround_time)/len(avg_turnaround_time),2*sum(avg_waiting_time)/len(avg_waiting_time),(0.7)*sum(percent_execution)/len(percent_execution),queue]) 